...
// Start from (0, 0): The rat starts at the top-left corner of the maze, and the goal is to reach the bottom-right corner (n-1, n-1).

// Recursive Backtracking: Use a recursive function to explore all possible moves (right, down, left, up) from the current position.

// Check Valid Moves: For each move, ensure it’s within bounds, the cell is not blocked (maze[x][y] == 1), and hasn’t been visited before.

// Mark Path: If a valid move is found, mark the current cell as part of the path, and recursively attempt to solve from that position.

// Backtrack if Stuck: If a move doesn’t lead to a solution, backtrack by unmarking the current cell and try a different path.
